$.fn.extend({
    ...
    artEditor: function (options) {
        ...
        try {
            ...
            //             return;
            //         }
            //         var image = '<img src="' + data + '" style="width:90%;" /><br><br>';
            //         _this.insertImage(image);
            //     };
            // });
              var options = _this._opt.qiniuOption(function(up, file, info){
              var domain = up.getOption('domain');
              var res = JSON.parse(info);
              var sourceLink = domain + res.key; //获取上传成功后的文件的Url
                 console.log("sourceLink:"+sourceLink);
                  var image = '<img src="' + sourceLink + '" style="width:90%;" /><br><br>';
                  _this.insertImage(image);
               });
            var  requestHead = {
                    token: '',
            ...
            $.ajax({
                ...
                type: 'GET',
                url: 'http://api.zaiyi.chengyiwm.com/getUploadInfo?bucketName=chengyiwm-test',
                data: {},
                dataType: 'JSON',
                success: function (data) {
                    var token = data.body.token;
                    options.uptoken = token;
    ...
    upload: function (data) {
        ...
        $.ajax({
            ...
            .then(function (res) {
                ...
                if (src) {
                    var img = '<img src="' + src + '" style="width:90%;" /><br><br>';
