<template>
    <div class="my-attention">
        ...
        <ul v-if="list.length==0" class="null-content">
            ...
            <div class="null-content-text">多发言,就能收获粉丝~</div>
        </ul>
        <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded"  ref="loadmore">
            <ul v-for="item in list" class="cell">
                <div  @click.stop="userHome(item.id)" class="cell-left">
    ...
    export default{
        ...
        methods: {
            getList:function (start,offset) {
                Indicator.open();
                API.$userFanList(UTIlS.getUserId(), start, offset, res=> {
                    UTIlS.log('我的粉丝列表', res);
                    var _this = this;
                    if (res.head.code == 0) {
                        Indicator.close();
